    ...
    for i in range(len(string_list1)):
        ...
        for j in range(len(string_list1[i])):
            if string_list1[i][j].isupper():
                p.append(string_list2[i][j].upper())
            else:
                p.append(string_list2[i][j].lower())
        if ''.join(p) != '':
            rez.append(''.join(p))
...
def func4(input_file, output_file):
    ...
    with open(input_file, mode='rt') as fr:
        for line in fr:
            rez.append(line.replace(',', ' ').split())
    
    for i in range(len(rez)):
        for j in range(len(rez[i])-1):
            rez[i][j] = str(int(rez[i][j]))+', '
    
    with open(output_file, mode='wt') as fw:
        for x in rez:
            fw.write('['+''.join(x)+']\n')
    return len(rez)*len(rez[0])
...
def func5(input_pngfile):
    ...
    for row in range(len(img)):
        for col in range(len(img[0])):
            if img[row][col] == (255,)*3:
                c_srt = col
                while col != len(img[0]) and img[row][col] == (255,)*3:
                    col+=1
                rez.append((row, c_srt, col-1))
                break
    return rez
...
def aux_2(root, l=0):
    ...
    if root.right:
        p = aux_2(root.right, l+1)
        for k, v in p.items():
            rez[k] = rez.get(k, '')+v
    
    if root.left:
        p = aux_2(root.left, l+1)
        for k, v in p.items():
            rez[k] = rez.get(k, '')+v
...
def ex2(root):
    rez = aux_2(root)
    return '-'.join(''.join(sorted(x)) for x in list(rez.values()))
