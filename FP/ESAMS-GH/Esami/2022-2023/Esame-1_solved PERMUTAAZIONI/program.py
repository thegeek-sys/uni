...
def func2(pathname):
    rez = {}
    with open(pathname, mode='rt') as fr:
        for line in fr:
            num = line.strip().split(',')
            num[0] = int(num[0])
            if num[1] in rez:
                rez[num[1]] = max(num[0], rez[num[1]])
            else:
                rez[num[1]] = num[0]
    return rez
...
def func3(listaA, pathname):
    listaA = sorted(listaA, key = lambda x: (-len(x), x), reverse=True)
    i = 0
    print(listaA)
...
# print(func3(lista, 'func3_1_out.txt'))
...
def func4(S):
    for x in S:
        if not x.isalpha():
            S = S.replace(x, ' ')
    wlist = [ x.lower() for x in S.split() ]
    M = len(max(wlist, key=len)) + 1
    wlist = sorted(wlist)
    print(wlist)
    
    rez = ''
    for x in sorted(set(wlist)):
        ast = wlist.count(x)
        rez += x + ' '*(M-len(x)) + '*'*ast + '\n'
...
def func5(img, output_file_name):
    ...
    for col in range(len(img[0])):
        ...
        for row in range(len(img)-1,-1,-1):
            p.append(img[row][col])
        new_im.append(p)
        
    images.save(new_im, output_file_name)
    return (len(new_im), len(new_im[0]))

# img_in  = 'func5/image01.png'
# img_out = 'func5/your_image01.png'

# print(func5(images.load(img_in), img_out))
...
def ex1(root):
    ...
    for file in o:
        ...
        with open(file, mode='rt') as fr:
            ...
            for x in fr:
                s += chr(int(x))
        rez[os.path.dirname(file)] = rez.get(os.path.dirname(file), '') + s
