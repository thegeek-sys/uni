...
def func5(points):
    dis = {}
    for point in points:
        dis[point] = round((point[0]**2+point[1]**2)**0.5, 3)
    
    check = []
    mins = sorted(v for v in dis.values())[0:3]
    for k, v in dis.items():
        if v in mins:
            check.append(k)
    
    return (round((check[0][0]+check[1][0]+check[2][0])/3, 3),round((check[0][1]+check[1][1]+check[2][1])/3, 3))
...
def aux1(root, l=0):
    rez = {}
    rez[l] = rez.get(l,0)+root.value
    
    if root.left:
        p = aux1(root.left, l+1)
        for x in p:
            rez[x] = rez.get(x, 0)+p[x]
    
    if root.right:
        p = aux1(root.right, l+1)
        for x in p:
            rez[x] = rez.get(x, 0)+p[x]
    ...
    for ls in os.listdir(dirin):
        ...
        if path.endswith('.txt'):
            ...
            for x in p:
                i = find(rez, x[0])
                if i != None:
                    rez[i] = rez[i][0],rez[i][1]+x[1]
                else:
                    rez.append((x[0],x[1]))
    return sorted(rez, key=lambda x: (-x[1], x))
                        

# directory = 'ex2'
# words = ["cat", "dog"]
# print(ex2(directory, words))
