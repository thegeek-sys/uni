...
def func1(diz1, diz2):
    ...
    for k, v in diz1.items():
        if k in diz2:
            l = set(v) ^ set(diz2[k])
            rez[k] = sorted(l, key= lambda x: (-len(x), x))
    return rez
...
def func3(textfile_in, textfile_out):
    with open(textfile_in, mode='rt') as fr:
        with open(textfile_out, mode='wt') as fw:
            fr = fr.read().split()
            fr = sorted(fr, key=lambda x: (-len(x.replace('-', '').replace('+', '').replace('.', '')), float(x)))
            # print(fr)
            fw.write(', '.join(fr))
...
def func4(input_filename):
    ...
    rez = []
    for col in range(len(out[0])-1, -1, -1):
        p = []
        for row in range(len(out)):
            p.append(out[row][col])
        rez.append(p)
                
    
    
    return [ x[::-1] for x in rez ]
...
def func5(txt_input, width, height, png_output):
    ...
    with open(txt_input, mode='rt') as fr:
        for l in fr:
            l = l.split()
            diagonals.append([l[0], (int(l[1]), int(l[2]), int(l[3])), (int(l[4]), int(l[5])), int(l[6])])
    
    dd, du = 0, 0
    for diagonal in diagonals:
        direction = diagonal[0]
        color = diagonal[1]
        x,y = diagonal[2][0], diagonal[2][1]
        l = diagonal[3]
...
def ex1(root : NaryTree, valori : list[int], l=0):
    i = 0
    
    root.value = root.value+valori[l]
    i += root.value
...
# root = [-7, [-10, [6], [-2]],[-3,  [9]],[-8,  [7]],[-10, [-9]],[-5]]
# values = [-42, -80, 68, 2, 81, 75, 54, 48, -4, 5]
# print(ex1(NaryTree.fromList(root.copy()), values))
...
def aux_2(dirin, words):
    rez = {}
    for ls in os.listdir(dirin):
        path = f'{dirin}/{ls}'
...
def ex2(dirin, words):
    count = aux_2(dirin, words)
    rez = []
    for k, v in count.items():
        rez.append((k,v))
    
    return sorted(rez, key=lambda x: (-x[1], x[0].lower()))
