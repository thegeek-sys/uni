def es24(ifile, l):
    ...
    with open(ifile, mode='rt') as fr:
        fr = [ x for x in fr.read().lower() if x.isalpha() ]
    return sorted([(x,str(round((100*fr.count(x.lower()))/len(fr), 2))[::-1].zfill(4)[::-1]+'%') for x in l], key=lambda x: (-float(x[1].strip('%')), x[0]))
