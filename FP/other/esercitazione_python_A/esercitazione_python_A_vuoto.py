...
def num_digits_neg(v):
    return len(str(v)) if v>=0 else len(str(-v))
...
def check_int_str(stringa, intero):
    ...
    if intero < 0:
        ...
        for i in range(len(stringa)-1):
            if stringa[i] == '-' and stringa[i+1] == intero:
                somma += 1
    else:
        intero = str(intero)
...
def count_sub_string(query, corpo):
    l = 0
    for i in range(len(corpo)-len(query)+1):
...
def check_reverse(S, T):
    return True if T[::-1] == S else False
...
-   la nuova lista out deve contenere il valore preso da L alla posizione indicata
    dal valore i-esimo delle lista `src`. Il valore preso da L deve essere scritto
    in out nella posizione indicata dall'elemento i-esimo della lista `dst`.
...
infatti 2 e' preso dalla posizione 1 di L e scritto in posizione 2 di out.
