...
def func1(D):
    rez = []
    for k, v in D.items():
        rez.append(sorted(v, reverse=True)[k])
    return rez
...
def func2(list_all, list_rm):
    for x in list_all[::-1]:
        if x not in list_rm or list_all.count(x) > 1:
            list_all.remove(x)


# lista_all = [2, 4, 3, 4, 4, 3, 4, 5, 2, 6]
# lista_rm = [5, 3, 2, 7]
# func2(lista_all, lista_rm)
# print(lista_all)
...
def func5(points):
    ...
    for x in points:
        # print(rez)
        dis = round((x[0]**2+x[1]**2)**0.5, 3)
        rez.append([dis, x])
    
    mins = sorted(rez, key = lambda x: x[0])[0:3]
    # print(mins)
    med_x = round((mins[0][1][0]+mins[1][1][0]+mins[2][1][0])/3, 3)
    med_y = round((mins[0][1][1]+mins[1][1][1]+mins[2][1][1])/3, 3)
...
def rec_ex1(root, l=0):
    rez = {}
    rez[l] = root.value
    
    if root.left:
        part = rec_ex1(root.left, l+1)
        for x in part:
            rez[x] = rez.get(x, 0)+part[x]
        
    if root.right:
        part = rec_ex1(root.right, l+1)
        for x in part:
            rez[x] = rez.get(x, 0)+part[x]
...
# root = BinaryTree.fromList([5, [8, None, [3, None, None]], [2, [9, None, None],[1, None, None]]])
# result = []
# ex1(root, result)
# print(result)
...
def ex2(dirin, words):
    ...
    files = rec_ex2(dirin)
    for f in files:
        with open(f, mode='rt') as fr:
            fr = fr.read()
            # print(fr)
            for word in words:
                rez[word] = rez.get(word, 0)+fr.count(word)
    
    rez = sorted([ (k,v) for k, v in rez.items() ], key = lambda x: (-x[1], x[0]))
    return rez
