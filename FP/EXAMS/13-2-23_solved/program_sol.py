...
def func1(D):
    ...
    '''
    risultato = []
    for k,lista in D.items():
        risultato.append(sorted(lista, reverse=True)[k])
    return risultato
    '''
    return [ sorted(lista, reverse=True)[k] for k,lista in D.items()   ]
...
def func2(list_all, list_rm):
    ...
    for i in range(len(list_all)-1,-1,-1):
        if list_all[i] in list_all[:i]:
            del list_all[i]

def func2(list_all, list_rm):
    risultato =  []
    for X in list_all:
        if X in list_rm and not X in risultato:
            risultato.append(X)
    list_all[:] = risultato
...
def func3(strList):
    # scrivi qui il tuo codice
    return [
        sum([ord(C) for C in parola], default=0) 
            for parola in sorted(strList)
        ]
...
def func5(points):
    ...
    ordinati = sorted(points, key=criterio)
    A, B, C = ordinati[:3]
    X = (A[0]+B[0]+C[0])/3
    Y = (A[1]+B[1]+C[1])/3
    return X,Y
...
def ex1(root, result):
    # scrivi qui il tuo codice
    somma_per_livelli(root, result, 0)

def somma_per_livelli(root, lista, profondità):
    if root is None:
        return
    else:
        if len(lista) > profondità:
            lista[profondità] += root.value
        else:
            lista.append(root.value)
    somma_per_livelli(root.left,  lista, profondità+1)
    somma_per_livelli(root.right, lista, profondità+1)
...
def ex2(dirin, words):
    ...
    # scrivi qui il tuo codice
    D = { parola : 0 for parola in words }
    esplora_directory(dirin, D)
    return sorted(list(D.items()), key=criterio)

def esplora_directory(dirin, D):
    for nome in os.listdir(dirin):
        fullname = dirin + '/' + nome
        if os.path.isdir(fullname):
            esplora_directory(fullname, D)
        elif nome.endswith('.txt'):
            with open(fullname, encoding='utf8') as F:
                parole = F.read().split()
            for parola in D:
                D[parola] += parole.count(parola)
