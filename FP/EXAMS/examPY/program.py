...
def ex1(triangles):
    ...
    for triangle in triangles[::-1]:
        sort_tri = sorted(triangle)
        if round((sort_tri[0]**2 + sort_tri[1]**2), 3) != round(sort_tri[2]**2 ,3):
...
def draw_rect(img, w, h, r, c):
    #h = dim[0]
    #w = dim[1]
    color = img[c][r]
    #print(color)
    max_r = min(r+w, len(img[0]))
    img[c][r:max_r] = [color]*w
    if img[min(c+h-1, len(img)-1)][r:max_r] == [(0,0,0)]*w:
        img[min(c+h-1, len(img)-1)][r:max_r] = [color]*w
    else:
        img[min(c+h-1, len(img)-1)][r:max_r-1] = [color]*(w-1)
    for col in range(c, min(c+h-1, len(img))):
        img[col][r] = color
        #if img[col][min(r+w, len(img[0]))-1] == (0,0,0):
        img[col][min(r+w, len(img[0]))-1] = color
    #images.visd(img)
    return img
...
def ex2(img_in, img_out, colors):
    ...
    for c in range(len(img)):
        for r in range(len(img[0])):
            if img[c][r] in colors:
                #images.visd(img)
                
                #print(img[c][r])
                #print(colors[img[c][r]][0])
                h = colors[img[c][r]][0]
                w = colors[img[c][r]][1]
                colors.pop(img[c][r])
                
                img = draw_rect(img, w, h, r, c)
                #num += 1
