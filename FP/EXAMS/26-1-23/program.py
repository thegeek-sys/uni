...
def func1(string_list1, string_list2):
    string_list1 = {x for x in string_list1}
    string_list2 = {x for x in string_list2}
    rez = string_list1 ^ string_list2
    return sorted(rez, key= lambda x: x, reverse=True)
...
def func2(path_to_file):
    rez = {}
    with open(path_to_file, mode='rt', encoding='utf-8') as fr:
        for i in fr:
            for j in i:
                rez[j] = rez.get(j, 0)+1
    return rez
...
def func3(a_list):
    base = len(a_list)
    M, m = max(a_list), min(a_list)
    for x in a_list[::-1]:
        if x == M or x == m:
            a_list.remove(x)
    return base-len(a_list)
...
def func4(input_filename, output_filename):
    with open(input_filename, mode='rt') as fr:
        fr = fr.read()
        no_rep = {x for x in fr.split()}
    no_rep = sorted(no_rep, key=lambda x: x.upper())
...
def func5(imagefile, output_imagefile, color):
    ...
    for r in range(len(img)):
        ...
        for c in range(len(img[0])):
            if img[r][c] == (255, 255, 255) and 'srt' not in locals():
                N_r = r
                srt = c
                seg += 1
            elif img[r][c] == (255, 255, 255):
                end = c
                seg += 1
        if seg > l[0][0]:
            l = [[seg, N_r, srt, end]]
        elif seg != 0 and seg == l[0][0]:
            l.append([seg, N_r, srt, end])
...
# output_imagefile = 'func5/out04.png'
# color = (0,0,0)
